...
def login_user(user):
    ...
    select = "SELECT * FROM users Where (username = '{}' and password = '{}')".format(
...
def check_open(user_id, password):
    mydb = connext_db()
    mycursor = mydb.cursor()
    select = "SELECT * FROM users Where (id = '{}' and password = '{}')".format(user_id,password)
    mycursor.execute(select)
    myresult = mycursor.fetchone()
    if(myresult):
        row = map_data_login(myresult)
...
def update_garage(garageId, status):
    ...
    update = "UPDATE garages SET status = '{}', parking_date = '{}' Where id = '{}'".format(
        status, dateNow, garageId)
